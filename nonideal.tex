...
paper by Giovangigli and collaborators \cite{giovangigli_CTM:2011,giovangigli:2011}.
...
which establishes the form of the thermodynamic forces and fluxes. We then use
the fluctuation-dissipation principle to formulate the corresponding noise terms.
Here we only need to consider the contributions of the heat flux and mass diffusion fluxes
...
Before discussing the form of the noise terms we will first recast $\OnsagerMatrixB$ in a slightly different form.
...
\]
We can get several variables this way
\[
...
Another issue is that it gives things as function of $\rho$, $T$ and $Y$.  One of the
...
\frac{1}{\bar{b}} ln ( 1 + \frac{\bar{b}}{\tau})
+ 
k_B T \sum \frac{Y_k}{m_k} \frac{\tau}{\tau -bar{b}} - \frac{\bar{a}}{\tau(\tau + \bar{b})}
\]
except he has a $\bar{b}$ in the numerator of the sum instead of $\tau$, which is iterated 
across several papers but seems inconsitent with $h = e + p \tau$.  So these need to be checked.
