...
\newcommand{\manu}[1]{{\bf[{\color{blue}{EM:  #1}}]}}
...
\begin{description}
    \item[JBB] \sout{Write out reaction rate formalism in form of \ref{eq:rxn_corr} and fix that equation}\\
        \rwg{This seems to be done for now.}
    \item[RWG] \sout{Check on Hari's availablity to work on Fuego / rxn rate corrections}\\
    \rwg{Hari's starting to adjust Fuego based on JBB's writeup, noting that things may yet evolve.}
    \item[MSD] \sout{Check if we actually need $c_v$ anywhere in PeleC} \\
      \msd{$c_v$ is required in (at least) two places: to compute the explicit timestep limit due to temperature diffusion, and to compute $T$ from $(e,Y)$.  The latter is done in the FUEGO-generated code, and uses a call to {\tt CKCVBS}}
    \item[SY] \sout{Generate plots of bulk and shear viscosity vs. T at multiple P (or vs. P at fixed T) from NIST database }\\
        \rwg{SY showed me these plots - upshot we seem to be fine for CH4-air and H2-air, for heavier fuels may need to revisit. If anyone would like to see them in this doc or elsewhere say so\ldots}
    \item[SY] \sout{Contact EM to discuss what has been done w.r.t. reading in $T_c$, $P_c$ for species and interfacing with LJ potentials to estimate those not available}
    \manu{EM has modified fuego in branch {\it fuego-work} of {\it PelePhysics}. Fuego is now creating a new routine in the $*.c$ file to provide the critical temperature as well as $a_i$, $b_i$ and acentric factor. There is also a dictionary with known values from NIST database instead of using formulas.}
    \item[HS] \sout{Set up branch for Fuego mods} \\
        \rwg{MSD did this: branch of {\it PelePhysics} is {\it fuego-work}}
...
\end{equation}
\jbb{I thought $Z$ was basically $p/p^{id}$.  in which case
\[
p - p^{id} = \Phi \rightarrow \Phi = (Z-1)p^{id} \rightarrow Z
= 1+\frac{\Phi \mbar}{\rho k_b T}
= 1+\frac{\Phi \overline{W}}{\rho R T}
\]
In (\ref{eq:DepartInternalEnegeryIntegral}), $R$ should be $R/\overline{W}$.  
(We should try to reserve $R$ for universal gas constant.)
...
\jbb{this is unncessarily obtuse.  $h = e + p/\rho$.}
...
Peng-Robinson (PR) EOS: $\delta = 2 b_{m}$, $\epsilon = -b_{m}^2$, $\Theta = a_{m}$ in the Eq.~\ref{eq:PReosCompressibility}:
\[
\label{eq:pPR}
p = 
\]

 In the case of PR EOS the species attractive and repulsion factors $a_m$ and $b_m$ are expressed in the form similar to the SRK EOS, shown in the Eq.~\ref{eq:abformEOS}. The temperature correction factor $\alpha(T/T_{c,i})$ has the same form as SRK EOS. The functional form of $f\left( \omega_i \right)$, expressed as a polynomial in terms of the acentric factor, has a different form compared to SRK EOS. 
\[
...
\end{equation}
%\jbb{Note that one does not need to introduce $Z$ to do this.  Integrals can be done by partical fractions
%for Peng Robinson using formalism above.  Just messier the log term below because roots of quadratic are not 
%as simple.}
where 
\[
...
\jbb{$ h = e + p/\rho = e + p \tau$}
\MarginPar{check minus sign in eq.}
\begin{equation}
h^{ig} - h = \left[ T \left .  \frac{\partial a_{m}}{\partial T} \right |_{\rho,Y}- a_{m}\right] K_{1} + (-?)\Phi\tau% (1 - Z) RT
\end{equation}
...
\]
\jbb{You do not need a complex derivation here.  Result follows directly from (\ref{eq:calc_cov}).}
\[
c_p = \left. \frac{\partial h_m}{\partial T} \right |_{Y,\rho}
- \frac {\frac{\partial h}{\partial \rho}} 
 {\frac{\partial p}{\partial \rho}}
\left. \frac{\partial p}{\partial T} \right |_{Y,\rho}
\]
...
\MarginPar{Subst. expressions for deriv. in eq at bottom of pg. 13}
\begin{equation}
c_{p} = c_{p}^{ig} -R -K_1 \left[T \frac{\partial^{2} a_{m}}{\partial T^{2}} \right] - T\left(\frac{\partial P}{\partial T}\right)^{2}_{\tau,Y}/ \left(\frac{\partial P}{\partial \tau}\right)_{T,Y}
\label{eq:CpRelation}
\end{equation}
...
%\]
%\jbb{This formula is general for all EOS that give $p$ as a function of $\rho$ and $T$. No special derviation 
%is needed for PR.  Just evaluate formula.} 
\MarginPar{Show that this is the same as $\Gamma_1$ based formula}
...
\jbb{The partial enthalpies can be computed directly from enthalpy using (\ref{eq:calc_cov})}
Specifically here
\[
\label{eq:hkPR}
h_k = \left. \frac{\partial h}{\partial Y_k} \right |_{T,\rho}
- \frac {\frac{\partial h}{\partial \rho}} 
 {\frac{\partial p}{\partial \rho}}
\left. \frac{\partial p}{\partial Y_k} \right |_{T,\rho}
\]
...
\MarginPar{Make consistent with \ref{sec:mix}}
     Following the discussion presented in~\cite{masquelet2013large}; the partial molar volumes can be computed using 
          \begin{align}
     V_{,l} &= \left(\frac{\partial \mathcal{V}}{\partial N_{l}} \right)_{T,P,N_{k,k\neq l}} \\
       ...
       \label{eq:molarf}
     \end{align}
     where $V$ is molar volume and $\mathcal{V}$ is total volume. 
     \begin{enumerate}
